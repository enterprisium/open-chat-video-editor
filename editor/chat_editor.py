...
class Text2VideoEditor(object):
    ...
    def run(self,input_text,style="",out_file="test.mp4"):
        ...
        if text_lang == 'zh':
            ...
            logger.info(f'zh_out_text: {zh_out_text}')

        en_out_text = [val['en'] for val in text_resp['out_text']]
        # if
        logger.info(f'en_out_text: {en_out_text}')
        # stylized text
        out_text_stylized = [f'{val},{style}' for val in en_out_text]
        ...
        for idx,(tts_info,vision_info,one_text) in enumerate(zip(tts_resp, vision_resp,sub_title_text)):
            ...
            # save for debug
            vision_clip.write_videofile(f"test_{idx}.mp4", fps=24)
            final_clips.append(vision_clip)
        logger.info(f'final_clips: {len(final_clips)}')
